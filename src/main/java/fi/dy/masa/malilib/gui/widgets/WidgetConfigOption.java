...
public class WidgetConfigOption
  extends WidgetConfigOptionBase<ConfigOptionWrapper> {
  ...
  public WidgetConfigOption(
    ...
    if (wrapper.getType() == ConfigOptionWrapper.Type.CONFIG) {
      ...
      if (config instanceof BooleanHotkeyGuiWrapper booleanHotkey) {
        ...
        this.initialKeybindSettings =
          config.getType() == ConfigType.HOTKEY
            ? ((IHotkey) config).getKeybind().getSettings()
            : null;
      } else {
        ...
        if (config instanceof IConfigStringList) {
          this.initialStringList =
            ImmutableList.copyOf(((IConfigStringList) config).getStrings());
        }
      }
  ...
  protected void addConfigOption(
    int x,
    int y,
    float zLevel,
    int labelWidth,
    ...
    if (config instanceof BooleanHotkeyGuiWrapper wrapper) {
      ...
      this.addBooleanAndHotkeyWidgets(
          x,
          y,
          configWidth,
          wrapper,
      ...
      ConfigButtonOptionList optionButton = new ConfigButtonOptionList(
        x,
        y,
        configWidth,
        configHeight,
        (IConfigOptionList) config
      );
      this.addConfigButtonEntry(
          x + configWidth + 2,
          y,
      ...
      ConfigButtonStringList optionButton = new ConfigButtonStringList(
        x,
        y,
        configWidth,
        configHeight,
      ...
      ConfigButtonColorList optionButton = new ConfigButtonColorList(
        x,
        y,
        configWidth,
        configHeight,
        (IConfigColorList) config,
        this.host,
        this.host.getDialogHandler()
      );
      this.addConfigButtonEntry(
      ...
      if (type == ConfigType.COLOR) {
        ...
        this.addWidget(
            new WidgetColorIndicator(
              this.colorDisplayPosX,
              y + 1,
              19,
              19,
              (IConfigInteger) config
            )
          );
      ...
      if (
        (type == ConfigType.INTEGER || type == ConfigType.DOUBLE) &&
        config instanceof IConfigSlider &&
        ((IConfigSlider) config).shouldUseSlider()
      ) {
        this.addConfigSliderEntry(
            x,
            y,
            resetX,
            configWidth,
      ...
      if (type != ConfigType.COLOR && config instanceof IConfigSlider) {
        IGuiIcon icon = ((IConfigSlider) config).shouldUseSlider()
          ? MaLiLibIcons.BTN_TXTFIELD
          : MaLiLibIcons.BTN_SLIDER;
        ButtonGeneric toggleBtn = new ButtonGeneric(
          this.colorDisplayPosX,
          y + 2,
          icon
        );
        this.addButton(
            toggleBtn,
            new ListenerSliderToggle((IConfigSlider) config)
          );
      }
    }
  }
    ...
    if (this.wrapper.getType() == ConfigOptionWrapper.Type.CONFIG) {
      ...
      if (config instanceof BooleanHotkeyGuiWrapper booleanHotkey) {
        ...
        if (this.textField != null) {
          modified |=
            this.initialStringValue.equals(
                this.textField.getTextField().getText()
              ) ==
            false;
        }
        ...
        if (
          this.initialKeybindSettings != null &&
          this.initialKeybindSettings.equals(
              ((IHotkey) config).getKeybind().getSettings()
            ) ==
          false
        ) {
        ...
        return (
          modified ||
          this.initialStringValue.equals(
              ((IStringRepresentable) config).getStringValue()
            ) ==
          false
        );
      } else if (
        this.initialStringList != null && config instanceof IConfigStringList
      ) {
        return (
          this.initialStringList.equals(
              ((IConfigStringList) config).getStrings()
            ) ==
          false
        );
      }
  ...
  public void applyNewValueToConfig() {
    if (
      this.wrapper.getType() == ConfigOptionWrapper.Type.CONFIG &&
  ...
  protected void addConfigComment(
    int x,
    int y,
    int width,
    int height,
  ...
  protected void addHotkeyConfigElements(
    int x,
    int y,
    int configWidth,
    String configName,
  ...
  protected void addBooleanAndHotkeyWidgets(
    int x,
    int y,
    int configWidth,
    ...
    ConfigOptionChangeListenerButton booleanChangeListener = new ConfigOptionChangeListenerButton(
      resettableConfig,
      resetButton,
      null
    );
    HotkeyedBooleanResetListener resetListener = new HotkeyedBooleanResetListener(
      resettableConfig,
      booleanButton,
      keybindButton,
      resetButton,
  ...
  protected void addConfigButtonEntry(
    ...
    ConfigOptionChangeListenerButton listenerChange = new ConfigOptionChangeListenerButton(
      config,
      resetButton,
      null
    );
    ConfigOptionListenerResetConfig listenerReset = new ConfigOptionListenerResetConfig(
      config,
      new ConfigResetterButton(optionButton),
      resetButton,
      null
    );
  ...
  protected void addConfigTextFieldEntry(
    ...
    GuiTextFieldGeneric field =
      this.createTextField(x, y + 1, configWidth - 4, configHeight - 3);
    ...
    ConfigOptionListenerResetConfig listenerReset = new ConfigOptionListenerResetConfig(
      config,
      new ConfigResetterTextField(config, field),
      resetButton,
      null
    );
  ...
  protected void addConfigSliderEntry(
    ...
    if (config instanceof IConfigDouble) {
      ...
      callback =
        new SliderCallbackInteger((IConfigInteger) config, resetButton);
    } else {
      return;
    }
  ...
  protected void addKeybindResetButton(
    int x,
    int y,
    IKeybind keybind,
    ConfigButtonKeybind buttonHotkey
  ) {
    ButtonGeneric button = this.createResetButton(x, y, keybind);

    ConfigOptionChangeListenerKeybind listener = new ConfigOptionChangeListenerKeybind(
      keybind,
      buttonHotkey,
      button,
      this.host
  ...
  public static class HotkeyedBooleanResetListener
    ...
    public HotkeyedBooleanResetListener(
      IConfigResettable config,
    ...
    public void actionPerformedWithButton(ButtonBase button, int mouseButton) {
      ...
      this.host.getButtonPressListener()
        .actionPerformedWithButton(button, mouseButton);
      this.updateButtons();
    }
