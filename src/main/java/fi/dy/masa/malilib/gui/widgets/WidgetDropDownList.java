...
public class WidgetDropDownList<T> extends WidgetBase {
  ...
  public WidgetDropDownList(
    ...
    int maxVisibleEntries,
    List<T> entries,
    @Nullable IStringRetriever<T> stringRetriever
  ) {
    ...
    if (width == -1) {
      ...
      for (int i = 0; i < entries.size(); ++i) {
        width =
          Math.max(
            width,
            this.getStringWidth(this.getDisplayString(entries.get(i))) + 20
          );
      }
    }
    ...
    if (this.isOpen && mouseY > this.y + this.height) {
      if (mouseX < this.x + this.width - this.scrollbarWidth) {
        ...
        if (this.scrollBar.wasMouseOver() == false) {
          ...
          int newPos = (int) (
            ((double) relY / (double) ddHeight) * this.scrollBar.getMaxValue()
          );
  ...
  @Override
  public boolean onMouseScrolledImpl(
    int mouseX,
    int mouseY,
  ...
  protected void updateFilteredEntries() {
    ...
    this.scrollBar.setMaxValue(
        this.filteredEntries.size() - this.maxVisibleEntries
      );
  }
  ...
  protected boolean entryMatchesFilter(T entry, String filterText) {
    return (
      filterText.isEmpty() ||
      this.getDisplayString(entry).toLowerCase().indexOf(filterText) != -1
    );
  }
  ...
  public void render(
    ...
    if (this.isOpen) {
      ...
      for (int i = startIndex; i < max; ++i) {
        ...
        if (
          mouseX >= this.x &&
          mouseX < this.x + this.width - scrollWidth &&
          mouseY >= y &&
