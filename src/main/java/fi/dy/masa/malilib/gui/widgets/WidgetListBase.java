...
public abstract class WidgetListBase<
  TYPE, WIDGET extends WidgetListEntryBase<TYPE>
>
  extends GuiBase {
  ...
  public boolean onMouseClicked(int mouseX, int mouseY, int mouseButton) {
    ...
    final int relativeY =
      mouseY - this.browserEntriesStartY - this.browserEntriesOffsetY;
        ...
        if (widget.isMouseOver(mouseX, mouseY)) {
          if (widget.canSelectAt(mouseX, mouseY, mouseButton)) {
            ...
            if (entryIndex >= 0 && entryIndex < this.listContents.size()) {
              this.onEntryClicked(
                  this.listContents.get(entryIndex),
                  entryIndex
                );
            }
          }
  ...
  protected boolean onMouseClickedSearchBar(
    int mouseX,
    int mouseY,
    int mouseButton
  ) {
    if (this.widgetSearchBar != null) {
      ...
      if (
        this.widgetSearchBar.onMouseClickedImpl(mouseX, mouseY, mouseButton)
      ) {
  ...
  public boolean onKeyTyped(int keyCode, int scanCode, int modifiers) {
    if (this.onKeyTypedSearchBar(keyCode, scanCode, modifiers)) {
      ...
      if (keyCode == KeyCodes.KEY_UP) this.offsetSelectionOrScrollbar(
          -1,
          true
        ); else if (
        keyCode == KeyCodes.KEY_DOWN
      ) this.offsetSelectionOrScrollbar(1, true); else if (
        keyCode == KeyCodes.KEY_PAGE_UP
      ) this.offsetSelectionOrScrollbar(
          -this.maxVisibleBrowserEntries / 2,
          true
        ); else if (
  ...
  protected boolean onKeyTypedSearchBar(
    int keyCode,
    int scanCode,
    int modifiers
  ) {
    if (
      ...
      this.widgetSearchBar.onKeyTyped(keyCode, scanCode, modifiers)
    ) {
      this.clearSelection();
      this.refreshBrowserEntries();
      this.resetScrollbarPosition();
  ...
  protected boolean onCharTypedSearchBar(char charIn, int modifiers) {
    if (
      this.widgetSearchBar != null &&
      this.widgetSearchBar.onCharTyped(charIn, modifiers)
    ) {
      this.clearSelection();
      this.refreshBrowserEntries();
      this.resetScrollbarPosition();
  ...
  public void drawContents(
    ...
    for (WIDGET widget : this.listWidgets) {
      ...
      boolean isSelected = this.allowMultiSelection
    ...
    if (
      hovered == null &&
      this.widgetSearchBar != null &&
      this.widgetSearchBar.isMouseOver(mouseX, mouseY)
    ) {
  ...
  protected void reCreateListEntryWidgets() {
    ...
    int usableHeight =
      this.browserHeight - this.browserPaddingY - this.browserEntriesOffsetY;
    int usedHeight = 0;
    int x = this.posX + 2;
    int y = this.posY + 4 + this.browserEntriesOffsetY;
    int index = this.scrollBar.getValue();
    WIDGET widget =
      this.createHeaderWidget(x, y, index, usableHeight, usedHeight);
    ...
    for (; index < numEntries; ++index) {
      widget =
        this.createListEntryWidgetIfSpace(
            x,
            y,
            index,
            usableHeight,
    ...
    this.scrollBar.setMaxValue(
        this.listContents.size() - this.maxVisibleBrowserEntries
      );
  }

  @Nullable
  protected WIDGET createListEntryWidgetIfSpace(
    int x,
    int y,
    int listIndex,
    int usableHeight,
    ...
    return this.createListEntryWidget(
        x,
        y,
        listIndex,
        (listIndex & 0x1) != 0,
  ...
  protected abstract WIDGET createListEntryWidget(
    int x,
    int y,
    int listIndex,
    boolean isOdd,
  ...
  protected void offsetSelectionOrScrollbar(
    int amount,
    ...
    if (changeSelection == false) {
      ...
      this.lastSelectedEntryIndex >= 0 && this.listContents.size() > 0
    ) {
      int index = MathHelper.clamp(
        this.lastSelectedEntryIndex + amount,
        0,
        this.listContents.size() - 1
      );

      if (index != this.lastSelectedEntryIndex) {
        if (
          index < this.scrollBar.getValue() ||
          index >= this.scrollBar.getValue() + this.maxVisibleBrowserEntries
        ) {
