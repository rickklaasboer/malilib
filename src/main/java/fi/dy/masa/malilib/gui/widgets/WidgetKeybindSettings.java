...
public class WidgetKeybindSettings extends WidgetBase {
  ...
  public void render(
    ...
    int edgeColor = this.keybind.areSettingsModified()
      ? 0xFFFFBB33
      : 0xFFFFFFFF;
  ...
  public void postRenderHovered(
    ...
    text.add(
      GuiBase.TXT_WHITE +
      GuiBase.TXT_UNDERLINE +
      StringUtils.translate(
        "malilib.gui.label.keybind_settings.title_advanced_keybind_settings"
      )
    );

    name =
      StringUtils.translate("malilib.gui.label.keybind_settings.activate_on");
    ...
    val =
      this.settings.getAllowEmpty()
        ? (GuiBase.TXT_GREEN + strYes)
        : (GuiBase.TXT_GOLD + strNo);
    text.add(String.format("%s: %s", name, val));
    ...
    val =
      this.settings.getAllowExtraKeys()
        ? (GuiBase.TXT_GREEN + strYes)
        : (GuiBase.TXT_GOLD + strNo);
    text.add(String.format("%s: %s", name, val));
    ...
    val =
      this.settings.isOrderSensitive()
        ? (GuiBase.TXT_GOLD + strYes)
        : (GuiBase.TXT_GREEN + strNo);
    text.add(String.format("%s: %s", name, val));
    ...
    name =
      StringUtils.translate("malilib.gui.label.keybind_settings.exclusive");
    val =
      this.settings.isExclusive()
        ? (GuiBase.TXT_GOLD + strYes)
        : (GuiBase.TXT_GREEN + strNo);
    text.add(String.format("%s: %s", name, val));
    ...
    String[] parts = StringUtils
      .translate("malilib.gui.label.keybind_settings.tips")
      .split("\\n");
