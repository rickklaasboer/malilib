...
public abstract class GuiBase
  extends Screen
  implements IMessageConsumer, IStringConsumer {
  ...
  protected static final String BUTTON_LABEL_ADD =
    TXT_DARK_GREEN + "+" + TXT_RST;
  protected static final String BUTTON_LABEL_REMOVE =
    TXT_DARK_RED + "-" + TXT_RST;
  ...
  public String getTitleString() {
    return (this.useTitleHierarchy && this.parent instanceof GuiBase)
      ? (((GuiBase) this.parent).getTitleString() + " => " + this.title)
      : this.title;
  }
  ...
  public boolean mouseScrolled(double mouseX, double mouseY, double amount) {
    if (
      ...
      Math.signum(amount) != Math.signum(this.mouseWheelDeltaSum)
    ) {
      this.mouseWheelDeltaSum = 0.0;
    }
  ...
  public boolean mouseReleased(double mouseX, double mouseY, int mouseButton) {
    if (
      this.onMouseReleased((int) mouseX, (int) mouseY, mouseButton) == false
    ) {
  ...
  public boolean charTyped(char charIn, int modifiers) {
    ...
    if (
      this.keyInputCount <= 0 && System.nanoTime() - this.openTime <= 100000000
    ) {
      this.keyInputCount++;
      return true;
    }
  ...
  public <T extends ButtonBase> T addButton(
    T button,
  ...
  public <T extends GuiTextFieldGeneric> TextFieldWrapper<T> addTextField(
    T textField,
    @Nullable ITextFieldListener<T> listener
  ) {
  ...
  public WidgetLabel addLabel(
    ...
    return this.addWidget(
        new WidgetLabel(x, y, width, height, textColor, lines)
      );
  }
  ...
  protected void drawButtonHoverTexts(
    int mouseX,
    int mouseY,
    float partialTicks,
    DrawContext drawContext
  ...
  protected void drawHoveredWidget(
    int mouseX,
    int mouseY,
    DrawContext drawContext
  ) {
  ...
  public void drawStringWithShadow(
    ...
    drawContext.drawText(this.textRenderer, text, x, y, color, true);
  }
  ...
  public int getMaxPrettyNameLength(List<? extends IConfigBase> configs) {
    ...
    for (IConfigBase config : configs) {
      width =
        Math.max(width, this.getStringWidth(config.getConfigGuiDisplayName()));
    }
