...
public class WidgetDirectoryNavigation extends WidgetSearchBar {
  ...
  public WidgetDirectoryNavigation(
    int x,
    int y,
    int width,
    int height,
    ...
    this.iconCreateDir =
      new WidgetIcon(x, y + 1, iconProvider.getIconCreateDirectory());
  }
  ...
  protected boolean onMouseClickedImpl(
    ...
    if (this.searchOpen == false) {
      ...
      if (hoveredIcon == this.iconRoot) {
        ...
        GuiTextInputFeedback gui = new GuiTextInputFeedback(
          256,
          title,
          "",
  ...
  public void render(
    ...
    if (this.searchOpen == false) {
      ...
      RenderUtils.drawRect(
        pathStartX,
        this.y,
        this.width - pathStartX - 2,
        this.height,
        0x20FFFFFF
      );

      int textColor = 0xC0C0C0C0;
      int maxLen = (this.width - 40) / this.getStringWidth("a") - 4; // FIXME // no
      String path = FileUtils.getJoinedTrailingPathElements(
        this.currentDir,
        this.rootDir,
        maxLen,
        " / "
    ...
    if (this.searchOpen == false) {
      ...
      if (hoveredIcon == this.iconRoot) {
        RenderUtils.drawHoverText(
          ...
          Arrays.asList(
            StringUtils.translate(
              "malilib.gui.button.hover.directory_widget.root"
            )
          ),
                drawContext
        );
        ...
        RenderUtils.drawHoverText(
          ...
          Arrays.asList(
            StringUtils.translate(
              "malilib.gui.button.hover.directory_widget.up"
            )
          ),
                drawContext
        );
        ...
        RenderUtils.drawHoverText(
          ...
          Arrays.asList(
            StringUtils.translate(
              "malilib.gui.button.hover.directory_widget.create_directory"
            )
          ),
                drawContext
        );
