...
public class WidgetColorListEditEntry extends WidgetConfigOptionBase<Color4f> {
  ...
  public WidgetColorListEditEntry(
    int x,
    int y,
    int width,
    int height,
    int listIndex,
    boolean isOdd,
    Color4f initialValue,
    Color4f defaultValue,
    WidgetColorListEdit parent
  ) {
    ...
    if (!this.isDummy()) {
      ...
      bx =
        this.addTextField(
            ...
            textFieldWidth,
            20,
            initialValue.toHexString()
          );

      this.addWidget(
          new WidgetColorIndicator(
            textFieldX + textFieldWidth + 2,
            y + 1,
            19,
            19,
            initialValue,
  ...
  protected int addTextField(
    ...
    ChangeListenerTextField listenerChange = new ChangeListenerTextField(
      field,
      resetButton,
      this.defaultValue.toString()
    );
    ListenerResetConfig listenerReset = new ListenerResetConfig(
      resetButton,
      this
    );
  ...
  protected ButtonGeneric createResetButton(
    int x,
    int y,
    GuiTextFieldGeneric textField
  ) {
  ...
  public boolean wasConfigModified() {
    return (
      !this.isDummy() &&
      !this.textField.getTextField().getText().equals(this.initialStringValue)
    );
  }
  ...
  private boolean canBeMoved(boolean down) {
    ...
    return (
      (this.listIndex >= 0 && this.listIndex < size) &&
  ...
  public static class ChangeListenerTextField
    ...
    public ChangeListenerTextField(
      GuiTextFieldGeneric textField,
      ButtonBase buttonReset,
      String defaultValue
    ) {
    ...
    public boolean onTextChange(GuiTextFieldGeneric textField) {
      this.buttonReset.setEnabled(
          !this.textField.getText().equals(this.defaultValue)
        );
      return false;
    }
  }
  ...
  private static class ListenerResetConfig implements IButtonActionListener {
    ...
    public ListenerResetConfig(
      ButtonGeneric buttonReset,
      WidgetColorListEditEntry parent
    ) {
    ...
    public void actionPerformedWithButton(ButtonBase button, int mouseButton) {
      this.parent.textField.getTextField()
        .setText(this.parent.defaultValue.toString());
      this.parent.parent.applyPendingModifications();
      this.buttonReset.setEnabled(
          !this.parent.textField.getTextField()
            .getText()
            .equals(this.parent.defaultValue)
        );
    }
  }
  ...
  private enum ButtonType {
    ...
    MOVE_DOWN(
      MaLiLibIcons.ARROW_DOWN,
      "malilib.gui.button.hovertext.move_down"
    );
